...
class TurmaFormBottomSheet extends StatefulWidget {
  final TurmaModel? turma;
  final TurmaViewModel vm;

  const TurmaFormBottomSheet({
    super.key,
    this.turma,
    required this.vm,
  });

  static void show(BuildContext context, TurmaViewModel vm, {TurmaModel? turma}) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      backgroundColor: Colors.transparent,
      builder: (_) => TurmaFormBottomSheet(turma: turma, vm: vm),
    );
  }

  @override
  State<TurmaFormBottomSheet> createState() => _TurmaFormBottomSheetState();
}

class _TurmaFormBottomSheetState extends State<TurmaFormBottomSheet> {
                                    ...
                                    child: Wrap(
                                      ...
                                      children: catequistas.map((c) => FilterChip(
                                        ...
                                        onSelected: (sel) => setState(() {
                                          if (sel) {
                                            _selectedCatequistas.add(c);
                                          } else {
                                            _selectedCatequistas.remove(c);
                                          }
                                        }),
                                      )).toList(),
                                    ),
